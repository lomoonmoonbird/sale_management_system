...
class AreaOverview(BaseHandler):
    def __init__(self):
        self.db = "sales"
    ...
    async def overview(self, request: Request):
        ...
        area_id = request['user_info'].get('area_id', '')
        channels = request.app['mongodb'][self.db][self.instance_coll].find({"parent_id": area_id, "role": Roles.CHANNEL.value, "status": 1}, {"_id": 1})
        channels = await channels.to_list(10000)
        channels_ids = [str(item['_id']) for item in channels]
        schools = request.app['mongodb'][self.db][self.instance_coll].find({"parent_id": {"$in": channels_ids}, "role": Roles.SCHOOL.value, "status": 1}, {"school_id": 1})
