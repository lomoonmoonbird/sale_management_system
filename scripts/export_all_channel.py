...
for channel in channels:
    try:
        ...
        wb = Workbook("./all_channels/%s.xlsx" % channel['name'])
        sheet_name = "data"
        ...
        for school_grade_stage_key, data in school_grade_stage_defaultdict.items():
            ...
            # print(sorted(data['date']['date'])[0], type(sorted(data['date']['date'])[0]))
            # print((datetime.datetime.now() - sorted(data['date']['date'])[0]))
            # print((datetime.datetime.now() - sorted(data['date']['date'])[0]).days)
            # print(channels_map.get(all_school_map.get(school_id, {}).get("owner_id", "")))
            # print(data.get("refund", {}), '---', data.get("refund", {}).get("amount", [0]))
            ws.append([
        ...
        print(11111)
        wb.save("./all_channels/%s.xlsx" % channel['name'])
    except:
        import traceback
        traceback.print_exc()
